        ...
        for temp_loc in self.multiworld.get_reachable_locations(state, self.player_id):
            ...
            try:
                if (temp_loc.address in self.missing_locations):
                    ...
                    is_hinted = temp_loc.address in self.hints
                    hint_suffix = ""
                    if is_hinted:
                        hint_data = self.hints[temp_loc.address]
                        ...
                        if item_name and receiving_player:
                            hint_suffix = f" **(Hinted: {item_name} for {receiving_player})**"
                        elif receiving_player:
                            hint_suffix = f" **(Hinted for {receiving_player})**"
                        else:
                            hint_suffix = " **(Hinted)**"
                    if self.output_format == "Both":
                        if temp_loc.progress_type == LocationProgressType.EXCLUDED:
                            ...
                            self.log_to_tab("[color="+self.get_ut_color("hinted") + "]" +region + " | " + temp_name + hint_suffix+"[/color]", True)
                            hinted_locations.append(temp_loc)
                        else:
                            self.log_to_tab(region + " | " + temp_name, True)
                        readable_locations.append(region + " | " + temp_name + hint_suffix)
                    elif self.output_format == "Location":
                        if temp_loc.progress_type == LocationProgressType.EXCLUDED:
                            self.log_to_tab("[color="+self.get_ut_color("excluded") + "]" +temp_name+"[/color]", True)
                        elif is_hinted:
                            self.log_to_tab("[color="+self.get_ut_color("hinted") + "]" +temp_name + hint_suffix+"[/color]", True)
                            hinted_locations.append(temp_loc)
                        else:
                            self.log_to_tab(temp_name, True)
                        readable_locations.append(temp_name + hint_suffix)
                    ...
                    try:
                        if (temp_loc.address in self.missing_locations):
                            ...
                            if self.enable_glitched_logic:
                                ...
                                if temp_loc.address in self.location_alias_map:
                                    temp_name += f" ({self.location_alias_map[temp_loc.address]})"
                                is_hinted_glitched = temp_loc.address in self.hints
                                hint_suffix_glitched = ""
                                if is_hinted_glitched:
                                    hint_data_g = self.hints[temp_loc.address]
                                    ...
                                    if item_name_g and receiving_player_g:
                                        hint_suffix_glitched = f" (Hinted: {item_name_g} for {receiving_player_g})"
                                    elif receiving_player_g:
                                        hint_suffix_glitched = f" (Hinted for {receiving_player_g})"
                                    else:
                                        hint_suffix_glitched = " (Hinted)"
                                if self.output_format == "Both":
                                    if temp_loc.progress_type == LocationProgressType.EXCLUDED:
                                        self.log_to_tab("[color="+self.get_ut_color("out_of_logic_glitched") + "]" +region + " | " + temp_name+"[/color]", True)
                                    elif is_hinted_glitched:
                                        self.log_to_tab("[color="+self.get_ut_color("hinted_glitched") + "]" +region + " | " + temp_name + hint_suffix_glitched+"[/color]", True)
                                        hinted_locations.append(temp_loc)
                                    else:
                                        self.log_to_tab("[color="+self.get_ut_color("glitched") + "]" +region + " | " + temp_name+"[/color]", True)
                                    readable_locations.append(region + " | " + temp_name + hint_suffix_glitched)
                                elif self.output_format == "Location":
                                    if temp_loc.progress_type == LocationProgressType.EXCLUDED:
                                        self.log_to_tab("[color="+self.get_ut_color("out_of_logic_glitched") + "]" +temp_name+"[/color]", True)
                                    elif is_hinted_glitched:
                                        self.log_to_tab("[color="+self.get_ut_color("hinted_glitched") + "]" +temp_name + hint_suffix_glitched+"[/color]", True)
                                        hinted_locations.append(temp_loc)
                                    else:
                                        self.log_to_tab("[color="+self.get_ut_color("glitched") + "]" +temp_name+"[/color]", True)
                                    readable_locations.append(temp_name + hint_suffix_glitched)
                            if region not in regions:
                                regions.append(region)
