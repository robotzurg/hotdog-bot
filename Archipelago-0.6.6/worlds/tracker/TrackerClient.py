    ...
    def updateTracker(self) -> CurrentTrackerState:
        ...
        if f"_read_hints_{self.team}_{self.slot}" in self.stored_data:
            ...
            for hint in self.stored_data[f"_read_hints_{self.team}_{self.slot}"]:
                if hint["status"] not in [HintStatus.HINT_FOUND, HintStatus.HINT_AVOID] and self.slot_concerns_self(hint["finding_player"]):
                    try:
                        ...
                        item_name = None
                    receiving_player = self.player_names.get(hint["receiving_player"], None)
                    hints[hint["location"]] = {
                        "status": hint["status"],
                        "item_name": item_name,
        ...
        if self.quit_after_update:
            ...
            if self.print_list:
                for i in sorted(updateTracker_ret.readable_locations, key=lambda x: 0 if "(Hinted" in x else 1):
                    logger.error(i)
            self.exit_event.set()
...
async def _wait_for_hints(ctx: TrackerGameContext) -> None:
    """In CLI quit mode, wait up to 2s for hints and datapackages before updating."""
    if ctx.quit_after_update and ctx.team is not None and ctx.slot is not None:
        hints_key = f"_read_hints_{ctx.team}_{ctx.slot}"
        for _ in range(40):
            if hints_key in ctx.stored_data:
                break
            await asyncio.sleep(0.05)
        # Wait for datapackages for all receiving players' games so item names resolve
        if hints_key in ctx.stored_data:
            needed_games = {
                ctx.slot_info[hint["receiving_player"]].game
                for hint in ctx.stored_data[hints_key]
                if hint.get("receiving_player") in ctx.slot_info
            }
            for _ in range(40):
                if all(game in ctx.checksums for game in needed_games):
                    break
                await asyncio.sleep(0.05)
